...
class Tour_m extends MY_Model {
	...
	{
		$sql = "SELECT r_tour.*,r_tour_detail.*, country_id, country_name, city_name, COALESCE(r_tour.photo_path,'".DEFAULT_TOUR_PHOTO_PATH."') as photo_path FROM r_country INNER JOIN r_city USING (country_id) INNER JOIN r_tour USING (city_id) INNER JOIN r_tour_detail USING(tour_id) WHERE tour_id = ? limit 1";
	...
	{
		$sql = "SELECT r_tour.tour_id, title, CONCAT(SUBSTRING(introduction,1,150),' ...') AS introduction, country_name, city_name, duration_hours, common_adult_price, COALESCE(r_tour.photo_path,'".DEFAULT_TOUR_PHOTO_PATH."') as photo_path FROM r_country INNER JOIN r_city USING (country_id) INNER JOIN r_tour USING (city_id) INNER JOIN r_tour_detail USING (tour_id) WHERE r_country.country_id = ? LIMIT ?,?"; 
	...
	{
		$sql = "SELECT r_tour.tour_id, title, CONCAT(SUBSTRING(introduction,1,150),' ...') AS introduction, country_name, city_name, duration_hours, common_adult_price, COALESCE(r_tour.photo_path,'".DEFAULT_TOUR_PHOTO_PATH."') as photo_path FROM r_country INNER JOIN r_city USING (country_id) INNER JOIN r_tour USING (city_id) INNER JOIN r_tour_detail USING (tour_id) WHERE r_city.city_id = ? LIMIT ?,?"; 
	...
	{
		$like_keyword='%'.$keyword.'%';
		$sql = "SELECT r_tour.tour_id, title, CONCAT(SUBSTRING(introduction,1,150),' ...') AS introduction, country_name, city_name, duration_hours, common_adult_price, COALESCE(r_tour.photo_path,'".DEFAULT_TOUR_PHOTO_PATH."') as photo_path FROM r_country INNER JOIN r_city USING (country_id) INNER JOIN r_tour USING (city_id) INNER JOIN r_tour_detail td USING (tour_id) WHERE td.title LIKE ? OR td.introduction LIKE ? OR td.highlight LIKE ? OR td.description LIKE ? LIMIT ?,?"; 
