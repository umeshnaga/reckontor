...
s.linkInternalFilters="javascript:,//viator.com,.viator.com"
...
function s_doPlugins(s) {
	...
	/* code for pagename plugin */
	if ((!window.s.pageType) && (!window.s.pageName || s.pageName==""))
		s.pageName=s.getPageName();

	/* code for setting pageview events */
	if (!s.events)
	{	
		s.events="event2";
	} else {
	...
	/* code for compute the difference between wishlist event and purchase event*/
...
/* Campaign Stacking - CVP Plugin */
if(!s.eVar62 && s.campaign)s.eVar62=s.crossVisitParticipation(s.campaign,'s_cpm','90','5',' > ','','1');
...
/*********************************************************************
* Function getTimeToComplete(v,cn,e): Returns the time to complete
*                 a task. When v is 'start' a cookie is written with
*                 the timestamp. When v is 'stop' the cookie is read
*                 and the expired time is returned in days, hours,
*                 minutes, or seconds.
*
*                 Days and Hours are rounded to .2 (e.g. 1.4 days),
*                 minutes to .5 (e.g. 2.5 minutes), and seconds to 5
*                 (e.g. 15 seconds).
*
*    v  = value - 'start' or 'stop'
*    cn = cookie name - like 'ttc'
*    e  = expiration - days to expiration of the cookie, 0 for session
* Returns:
*     '' or a value
*********************************************************************/
/*
...
s.visitorNamespace="viator"
s.trackingServer="bb.viator.com"
s.trackingServerSecure="sbb.viator.com"
